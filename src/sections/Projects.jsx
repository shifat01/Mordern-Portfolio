...
export default function Projects(){
  ...
  const {scrollYProgress} = useScroll({ //useScroll say that how much scroll happened
    target:scenRef,
      ...
      <div className="sticky top-0 h-screen flex flex-col items-center justify-center">
              ...
              <AnimatePresence mode="wait">
                {activeIndex === idx && (
                  <motion.h3 key={project.title}
                    ...
                    className={`block text-center text-[clamp(2rem,6vw,5rem)] text-white/95 sm:absolute sm:top-3 sm:left-[35%] lg:left-[-5%] sm:mb-0
                      italic font-semibold ${isMobile ? "-mt-24": "-mt-24"}
                    `}
                    style={{
                      zIndex: 5,
        ...
        <div className={`absolute ${isMobile ? "bottom-20": "bttom-10"}`}>
          <a href={activeProject?.link}
            ...
            className="inline-block mt-130 px-6 py-3 font-semibold rounded-lg bg-white text-black hover:bg-gray-200 transition-all"
